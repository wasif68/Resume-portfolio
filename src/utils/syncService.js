...
// PUBLIC fetch — no login needed, grabs the first resume row
export async function fetchResumeData(user, defaultData) {
  ...
  return { data: defaultData || defaultResumeData, source: "default" };
}
 
 
// CREATE resume if not exists for admin
export async function ensureResumeExists(user) {
  ...
  if (!data) {
    const local = localStorage.getItem(RESUME_KEY);
    const resumeData = local ? JSON.parse(local) : defaultResumeData;
